...
seed =  1234 #111 #1234 #420, 10601, 11741, 11641, 11747, 42, 69
...
def train(model, iterator, criterion, optimizer):
    ...
    for batch_idx, ele in enumerate(iterator):
        ...
        y_pred, e1, e2 = model.forward(x_batch_input_ids, x_batch_token_ids, x_batch_mask_ids, x_batch_input_ids2, x_batch_token_ids2, x_batch_mask_ids2, feats)
        is_deny = (y_batch == 2)
        cosine_loss = F.cosine_similarity(e1, e2)
        cosine_loss = ((cosine_loss > 0) * cosine_loss)
        deny_loss = is_deny.unsqueeze(1) * cosine_loss
        # print(deny_loss.sum().item())
        # print(criterion(y_pred, y_batch).item())
        loss = (criterion(y_pred, y_batch) + deny_loss.sum())/ (BATCH_SIZE * UPDATE_STEPS)
...
def validate(model, iterator, criterion):
    ...
    with torch.no_grad():
        for batch_idx, ele in enumerate(iterator):
            ...
            y_batch = ele.labels.to(device)
            y_pred, _, _ = model.forward(x_batch_input_ids, x_batch_token_ids, x_batch_mask_ids, x_batch_input_ids2, x_batch_token_ids2, x_batch_mask_ids2, feats)
